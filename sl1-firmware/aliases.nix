...
in [
  ...
  (alias "setup-rust-esp32" ''
    ${pkgs.espup}/bin/rustupespup install
    ${pkgs.rustup}/bin/rustuprustup override set esp
    chmod +x $HOME/export-esp.sh
    source $HOME/export-esp.sh
  '')
  (alias "setup-rust-esp32c3" ''
    ${pkgs.rustup}/bin/rustup toolchain install nightly \
      && ${pkgs.rustup}/bin/rustup override set nightly \
      && ${pkgs.rustup}/bin/rustup target add riscv32imc-unknown-none-elf \
      && ${pkgs.rustup}/bin/rustup component add rust-analyzer clippy rust-src
  '')
]
